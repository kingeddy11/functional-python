...
instead, you must use either the map and filter functions or list comprehens
ions.
'''
...
def threes(n):
    '''
    Returns a list of all numbers from 0 to n inclusive that contain the dig
    it 3.
    ...
    '''
    return [x for x in range(n+1) if x % 10 == 3 or x >= 30 and x < 40]


def small_words(text):
    '''
    Returns a list of all words in the input text that are less than 4 chara
    cters long.
...
def lengths(strings):
    '''
    Given a list of strings, returns a list of the lengths of the correspond
    ing strings.
